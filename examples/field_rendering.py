...
def rendering_field(
    ...
    max_val = torch.max(t_starts) + torch.max(t_ends)
    t_starts = torch.flip(max_val - t_starts, dims=[0])
    t_ends = torch.flip(max_val - t_ends, dims=[0])
    sigmas = torch.flip(sigmas, dims=[0])
    weights_rev, _, _ = render_weight_from_density(
    t_ends,
    t_starts,
    sigmas,
    ray_indices=torch.flip(ray_indices, dims=[0]),
    n_rays=n_rays,
    )
    # import ipdb; ipdb.set_trace()
    weights_rev = torch.flip(weights_rev, dims=[0])
    # extras["r_weights"] = weights_rev
    return colors, opacities, depths, weights, weights_rev
